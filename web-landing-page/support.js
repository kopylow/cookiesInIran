...
(function () {
  ...
  // ---------------------------------------------------------------------------
  // EDIT THESE before deploying. The Revolut/PayPal QR images are generated by
  // build_support_qr.py — change those addresses/links there too and re-run it,
  // or the visible value and its QR will disagree. The crypto QRs are hand-made
  // branded wallet exports (logo-in-center) under support/qr/*.jpg, verified to
  // decode to the addresses below; swap the image if an address changes.
  ...
  const PAYMENTS = {
    ...
    crypto: [
      // BTC has two receive formats. Both are shown (labelled) in the modal, each
      // with its own branded QR (logo-in-center wallet exports). The QR images
      // encode the bare address, which every major wallet scans.
      { key: "btc", label: "Bitcoin", symbol: "₿",
        addresses: [
          { label: "Taproot", address: "bc1p59dgjxy6u8pgvmchdhnmeu4swsfc7nvvw393te56kczadtx63kfssy4xqe", qr: "support/qr/btc-taproot.jpg" },
          { label: "Native SegWit", address: "bc1qlmzpu3neanphpwpl7m0edqevfy45340sutq37j", qr: "support/qr/btc-segwit.jpg" },
        ] },
      { key: "eth", label: "Ethereum", symbol: "Ξ", address: "0x1C8A2d42a66DF41C3a0467D659f23d6f8b8A59b2", qr: "support/qr/eth.jpg" },
      { key: "sol", label: "Solana", symbol: "◎", address: "7gf3oeu38riHM2g8rTtuFYw6vUeEFDQbkVjTVVwDd8no", qr: "support/qr/sol.jpg" },
    ],
    amazon: {
    ...
    PAYMENTS.crypto.forEach((c) => {
      crypto._grid.appendChild(triggerCard(LOGOS[c.key] || c.symbol, c.key, c.label, (body) => {
        if (Array.isArray(c.addresses)) {
          // Multiple receive formats (e.g. BTC Taproot + Native SegWit), each
          // labelled and paired with its own QR directly beneath the address.
          c.addresses.forEach((a) => {
            body.appendChild(copyRow(a.label, a.address, a.address));
            if (a.qr) body.appendChild(modalQr(a.qr, `${c.label} ${a.label} QR`));
          });
        } else {
          body.appendChild(copyRow(null, c.address, c.address));
          if (c.qr) body.appendChild(modalQr(c.qr, `${c.label} QR`));
        }
      }));
    });
