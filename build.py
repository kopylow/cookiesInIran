...
def inject_seo_fallback(de_html: str):
    """Mirror the German manuscript text into the <noscript> block of index.html so
    crawlers (and JS-less clients) see real content, not just the JS-loading shell.
    The book text is otherwise fetched client-side, leaving the initial HTML empty."""
    index_path = WEB_DIR / "index.html"
    if not index_path.exists():
        return

    # Keep only text-bearing markup; drop UI-only airlock image divs and discuss buttons.
    text_html = re.sub(r'<div class="airlock[^>]*></div>\s*', '', de_html)
    text_html = re.sub(r'<div class="discuss-wrapper">.*?</div>\s*', '', text_html, flags=re.DOTALL)

    start = "<!-- SEO-FALLBACK-START (auto-generated by build.py; do not edit by hand) -->"
    end = "<!-- SEO-FALLBACK-END -->"
    html = index_path.read_text(encoding="utf-8")
    pattern = re.compile(re.escape(start) + r".*?" + re.escape(end), flags=re.DOTALL)
    if not pattern.search(html):
        print("  [WARN] SEO-Fallback-Marker in index.html nicht gefunden; übersprungen.")
        return
    html = pattern.sub(start + "\n" + text_html + "\n                    " + end, html)
    index_path.write_text(html, encoding="utf-8")
    print("  [OK] SEO-Fallback in index.html aktualisiert.")
...
def main():
    ...
    for lang in ["de", "en", "ru", "fa"]:
        ...
        if lang != "fa":
            html = md_to_html(content, lang)
            (WEB_DIR / f"manuscript_{lang}.html").write_text(html, encoding="utf-8")
            if lang == "de":
                inject_seo_fallback(html)
        build_pdf(lang)
