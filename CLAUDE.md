...
- **QR codes**: split by method.
  - **Revolut + PayPal** are generated by `build_support_qr.py` (repo root) into
    ...
    changes.
  - **Crypto** (BTC Taproot + Native SegWit, ETH, SOL) uses hand-made branded wallet exports
    (logo-in-center) at `web-landing-page/support/qr/{btc-taproot,btc-segwit,eth,sol}.jpg`, NOT the
    generator. They encode the bare address (not a `scheme:` URI); each was verified with
    `zbarimg -q --raw` to decode to the address in `support.js`. If a crypto address changes, replace
    the matching `.jpg` and the address in `support.js` by hand, and re-verify the decode — do not
    re-add it to `build_support_qr.py`.
